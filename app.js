 ...
 * - Browser calls go to a public JSON-RPC endpoint (CORS + rate limits).
 * - Official public RPCs can throttle or block heavy traffic; swap RPC_URL
 *   for a free tier from Helius / QuickNode / Alchemy when needed.
 * - No backend: API keys must never be committed if the provider requires them
 *   in query strings that would be visible client-side.
...
(function () {
  ...
  /** Primary mainnet RPC (public). Override for production traffic. */
  const RPC_URL = "https://api.mainnet-beta.solana.com";

  /** Fallback RPCs tried if the primary fails (network / CORS / 429). */
  const RPC_FALLBACKS = [
  ...
  async function rpcGetBalance(address, endpoint) {
    ...
    if (response.status === 429) {
      const err = new Error("RPC rate limit reached (HTTP 429). Try again shortly or use your own RPC URL.");
      err.code = "RATE_LIMIT";
      throw err;
    }

    if (!response.ok) {
      const err = new Error(`RPC HTTP ${response.status} from ${endpoint}`);
      err.code = "HTTP";
      throw err;
    }
